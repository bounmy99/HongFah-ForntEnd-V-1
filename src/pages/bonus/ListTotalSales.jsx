...
import { Paybonus } from '../../functions/Bonus';
...
const ListTotalSales = () => {
  ...
  const user_id = selectableRow.map((item) => item.user_id);
    ...
    <div className="card-main">
      <div className="employee-table">
        <div class="employee-card-header">
          <div class="button">
            <div class="btn-show">
              <button type="button" class={`btn ${tablechange === 1 ? 'active' : ''}`} onClick={() => handleClick(1)}>ຮັກສາຍອດແລ້ວ</button>
            </div>
            <div class="btn-show">
              <button type="button" class={`btn ${tablechange === 2 ? 'active' : ''}`} onClick={() => handleClick(2)}>ບໍ່ຮັກສາຍອດ</button>
            </div>
            <div class="btn-show">
              {statusClick ?
                <button type="button" class={`btn`} onClick={handleClickTran}>ຈ່າຍໂບນັດ</button>
                :
                ""
              }
            </div>
          ...
          <>
            {tablechange === 1 && <MaintainTrue setSelectableRow={setSelectableRow} setStatusClick={setStatusClick}/>}
            {tablechange === 2 && <MaintainFalse setSelectableRow={setSelectableRow} setStatusClick={setStatusClick}/>}
          </>
      </div>
    </div>
  )
