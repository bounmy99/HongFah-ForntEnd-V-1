...
const HistoryTransfer = () => {
  ...
  useEffect(() => {
    GetAllBonus(users.token, "true").then(res => {
    ...
    <div className="card-main">
      <div className="employee-table">
        <div class="employee-card-header">
          ...
          <div class="button">
            <div class="btn-show">
              <input type="file" hidden className="input-field" name="file" onChange={handleImport}
                accept=".csv, application/vnd.openxmlformats-officedocument.spreadsheetml.sheet, application/vnd.ms-excel"
              />
              <button type="button" style={{ marginRight: 5 }} class="btn" onClick={() => document.querySelector(".input-field").click()}>Import ຂໍ້ມູນ</button>

              <button type="button" onClick={handleExport} class="btn">Export ທັງໝົດທີ່ເລຶອກ</button>
            </div>
          </div>
        </div>
        {emptyData ?
