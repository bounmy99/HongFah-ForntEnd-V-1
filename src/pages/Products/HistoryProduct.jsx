...
const HistoryProduct = () => {
  ...
  const columns = [
    {
      ...
      cell: row => (
        <div className="name-product">
          <Image src={row.products[0] && row.products[0].image} alt={row.products.name} width={50} height={50} />
        </div>
      ),
      sortable: true,
      width: '100px'
    ...
    },
    {
      ...
      cell: row => (
        <div className="status-score-history">
          {`${row.admin && row.admin.firstName} ${row.admin && row.admin.lastName}`}
        </div>
      ),
      width: '162px'
    },
    {
      ...
      cell: row => (
        <div className="status-status">
          <Tooltip title="ກົດເພື່ອເບິ່ງລາຍລະອຽດ" color="#00A5E8" placement="topRight">
          <Link to={`/listProducts/DetailProductSale/${row._id}`} style={{ textDecoration: 'none' }}>
            <EyeOutlined style={{ color: '#00A4CD', fontSize : 20, margin : 5 }}/>
          </Link>
          </Tooltip>
        </div>
      ),
  ...
  useEffect(() => {
    setLoading(true)
    GetAllOrderAdmin(users.token).then(res => {
      setLoading(false)
